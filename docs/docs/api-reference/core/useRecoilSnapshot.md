...
此钩子函数在渲染期间同步返回一个 [`Snapshot`](/docs/api-reference/core/Snapshot) 对象，并为所有 Recoil 状态更改订阅调用组件。你也许想将这个钩子用于调试工具，或者在初始渲染期间就需要同步拥有状态的地方，用于服务器端渲染，

```jsx
function useRecoilSnapshot(): Snapshot
```

请慎重使用此钩子，因为它会导致组件重新渲染 *所有* 的 Recoil 状态变化。今后，我们希望能够为业绩提供拆分能力。未来，我们希望能为提升性能提供防抖能力。
